...
async def get_admin_analytics(db: AsyncSession, slow_threshold_ms: int = 1000) -> dict:
    """
    Calculates query performance KPIs, feedback summary, frequent queries,
    and identifies slow/failed queries for admin review.
    """
    # 1. Base counts
    total_queries_res = await db.execute(select(func.count(QueryHistory.id)))
    total_queries = total_queries_res.scalar() or 0
    
    # 2. Query Status metrics
    status_stats_res = await db.execute(
        select(QueryHistory.status, func.count(QueryHistory.id)).group_by(QueryHistory.status)
    )
    status_stats = status_stats_res.all()
    ...
    feedback_stats_res = await db.execute(
        select(QueryHistory.feedback, func.count(QueryHistory.id)).group_by(QueryHistory.feedback)
    )
    feedback_stats = feedback_stats_res.all()
    ...
    speed_stats_res = await db.execute(
        select(
    ...
    speed_stats = speed_stats_res.first()
    ...
    freq_queries_res = await db.execute(
        select(
    ...
    freq_queries = freq_queries_res.all()
    ...
    slow_queries_res = await db.execute(
    ...
    slow_queries = slow_queries_res.scalars().all()

    # 7. Failed queries
    failed_queries_res = await db.execute(
        select(QueryHistory).filter(
    ...
    failed_queries = failed_queries_res.scalars().all()
...
async def optimize_query_by_id(db: AsyncSession, query_id: int) -> dict:
    """
    ...
    """
    record_res = await db.execute(select(QueryHistory).filter(QueryHistory.id == query_id))
    ...
    connection = connection_res.scalars().first()
    ...
    try:
        optimization_insights = await run_optimize_chain(
            dialect=dialect,
